...
WHY THIS FILE EXISTS:
    The instruction doc requires Category 1 (Privacy & Inference) specific metrics:
    - Attack success rate
    - Reconstruction MSE (gradient inversion)
    - Membership inference AUC

    This file implements all three so main.py can call them after training.

HOW IT CONNECTS:
    main.py imports run_privacy_attacks() and calls it after each experiment.
    Results are logged via utils.py ResultsLogger.

WHAT EACH ATTACK TESTS:
    1. Gradient Inversion (DLG-style): Can an attacker reconstruct input data
       from gradients? DP-SCAFFOLD/ULDP-AVG should be harder to attack.

    2. Membership Inference: Can an attacker tell if a sample was in training?
       Measured by AUC of a simple shadow model attack.

    3. Model Inversion: Can attacker reconstruct a "representative" image
       for each class from the model alone?
"""
