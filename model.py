...
WHY THIS FILE EXISTS:
    Both papers test on MNIST (logistic regression + DNN), FMNIST, and CIFAR-10.
    We need one central place that returns the right model for the right dataset.
    The instruction doc says to use these datasets, so we support all of them here.

WHAT THIS FILE DOES:
    1. LogisticRegression  — used in Paper 1 (DP-SCAFFOLD) for convex experiments
    2. SimpleDNN           — one hidden layer, used in Paper 1 for non-convex experiments
    3. SmallCNN            — used for FMNIST and CIFAR-10
    4. get_model()         — factory function, call this from client.py

HOW IT CONNECTS:
    client.py calls get_model(dataset_name) to get the right model.
    The model is then trained locally and its parameters are sent to the server.
"""
