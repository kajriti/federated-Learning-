...
# -----------------------
# FIXES APPLIED:
# 1. REMOVED fl.simulation.start_simulation() entirely — this was causing the crash.
#    On Windows, Ray (used internally by Flower simulation) has multiprocessing issues.
# 2. Replaced with a manual training loop that calls client.fit() directly.
#    This is functionally IDENTICAL — just no Ray in the middle.
# 3. Added proper if __name__ == "__main__": guard (required on Windows).
# 4. Added full traceback printing so you see the real error if something fails.
# """
...
# # ============================================================
# # MANUAL FL ROUND (replaces fl.simulation.start_simulation)
# # ============================================================
...
#     No Ray, no multiprocessing, no Flower simulation engine.
...
#         # DP-SCAFFOLD sends model params + global control variate c
...
#     if algorithm in ("dp_fedavg", "dp_scaffold", "uldp_avg"):
#         dp = config.get("dp", {})
#         print(f"  DP         : sigma={dp.get('noise_multiplier')}, C={dp.get('clipping_norm')}")
#     print(f"{'='*60}\n")

#     # Setup
#     logger    = ResultsLogger(exp_name_full, results_dir=results_dir)
#     init_model = get_model(dataset, config.get("model_type", "default"))
#     model_size = get_model_size_mb(init_model)
#     logger.set_model_size(model_size * (2 if algorithm == "dp_scaffold" else 1), clients_per_round)

#     global_params = get_parameters(init_model)
#     c_global      = [np.zeros_like(p) for p in global_params] if algorithm == "dp_scaffold" else None

#     round_accuracies, round_losses, round_epsilons = [], [], []
...
#         logger.log_round(
#             round_num=rnd, algorithm=algorithm, dataset=dataset,
#             num_clients=num_clients, alpha=alpha,
#             global_accuracy=acc, global_loss=loss,
#             epsilon=avg_eps, train_loss=avg_tl,
#         )
...
#     return {
#         "config": config, "summary": summary, "csv_path": csv_path,
...
#     if quick:
#         experiments = [
#             {"algorithm": "fedavg",      "num_rounds": 5},
#             {"algorithm": "dp_fedavg",   "num_rounds": 5},
#             {"algorithm": "dp_scaffold", "num_rounds": 5},
#             {"algorithm": "uldp_avg",    "num_rounds": 5},
#         ]
...
#         experiments = [
#             {"algorithm": a, "dataset": d, "num_clients": n, "alpha": al, "num_rounds": 200}
#             for a in algorithms for d in datasets for n in n_clients for al in alphas
#         ]
...
#             key = f"{config['algorithm']}_{config['dataset']}_n{config['num_clients']}_a{config['alpha']}"
...
#     _generate_comparison_plots(all_results, results_dir)
#     generate_results_table(all_summaries, save_path=os.path.join(results_dir, "results_table.csv"))
#     print(f"\n  Done! Results in: {results_dir}/")


# def _generate_comparison_plots(all_results, results_dir):
...
#                 "epsilon":         result.get("round_epsilons", [0] * len(rounds)),
#             })

#     if not algo_dfs:
#         print("  Nothing to plot."); return

#     plot_accuracy_vs_rounds(algo_dfs,
#         title="Figure 1: Global accuracy vs. communication rounds",
#         save_path=os.path.join(figures_dir, "figure1_accuracy_vs_rounds.png"))
#     plot_loss_vs_rounds(algo_dfs,
#         title="Figure 2: Global loss vs. communication rounds",
#         save_path=os.path.join(figures_dir, "figure2_loss_vs_rounds.png"))
#     plot_baseline_vs_methods(algo_dfs,
#         save_path=os.path.join(figures_dir, "figure3_baseline_vs_methods.png"))
#     dp_only = {k: v for k, v in algo_dfs.items() if k != "fedavg"}
#     if dp_only:
#         plot_epsilon_vs_accuracy(dp_only,
#             save_path=os.path.join(figures_dir, "figure4_privacy_utility_tradeoff.png"))


# # ============================================================
# # WINDOWS REQUIRES THIS GUARD — do not remove
# # ============================================================
# if __name__ == "__main__":

#     parser = argparse.ArgumentParser(description="FL Experiment Runner — Privacy & Inference")
...
#                         help="5 rounds, all 4 algorithms, MNIST, 10 clients")
#     parser.add_argument("--run_all",          action="store_true",
#                         help="Full matrix (many hours)")
...
#                 {algo: df},
#                 title=f"Accuracy vs rounds: {ALGORITHM_LABELS.get(algo, algo)}",
